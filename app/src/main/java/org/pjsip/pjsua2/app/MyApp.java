...
interface MyAppObserver
{
	...
	void notifyChatReceived(String fromUri, String toUri, String contentType, String messagebody);
	void notifySipMessageStatus(String reson, String toUri, String messagebody,int code);
}
...
class MyLogWriter extends LogWriter
{
    ...
    {
		try {
			Log.i("Log Message :","Log Message :"+entry.getMsg());
			if(entry.getMsg().contains("registration success") || entry.getMsg().contains("unregistration success")){
                Appreference.printLog("sipregister", entry.getMsg(), "DEBUG", null);
            } else if(entry.getMsg().toLowerCase().contains("call") || entry.getMsg().toLowerCase().contains("summary")) {
                Appreference.printLog("sipmessage", entry.getMsg(), "DEBUG", null);
            }
		} catch (Exception e) {
			e.printStackTrace();
		}
