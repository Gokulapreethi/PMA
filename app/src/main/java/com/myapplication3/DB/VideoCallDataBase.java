...
import com.myapplication3.Bean.From;
import com.myapplication3.Bean.Group;
import com.myapplication3.Bean.ListAllTaskBean;
import com.myapplication3.Bean.ListMembers;
import com.myapplication3.Bean.ListObserver;
import com.myapplication3.Bean.ListTaskFile;
...
import com.myapplication3.Bean.ProjectOrganisationBean;
import com.myapplication3.Bean.TaskDetailsBean;
import com.myapplication3.Bean.ToUser;
import com.myapplication3.Bean.User;
...
public class VideoCallDataBase extends SQLiteOpenHelper {
    ...
    public static final String CREATE_TABLE_PROJECT_STATUS="create table if not exists projectStatus(id integer primary key autoincrement,userId integer,projectId integer,taskId integer,fileName varchar(100),travelStartTime varchar(100),activityStartTime varchar(100),activityEndTime varchar(100),travelEndTime varchar(100),signature varchar(100),remarks varchar(100),observation varchar(100),hourMeterReading varchar(100),status varchar(100),activity varchar(100),customerName varchar(100),address varchar(100),McModel varchar(100),mcSrNo varchar(100),estimatedTravelHrs integer, estimatedActivityHrs integer,taskDescription varchar(100))";
...
//                cv.put("addmyagenda", bean.getMyagenta());
//                cv.put("cmesurveyfinished",sessionbean.getCmesurveyfinished());
            Log.i("ContactTable", "Insert");
...
//            cv.put("coordinator", bean.getCoordinator());
//            cv.put("chatmembers", bean.getChatmembers());
//            cv.put("conferenceuri", bean.getConferenceuri());
            if (isAgendaRecordExists("select * from contact where userid='" + bean.getId() + "'and loginuser='" + name + "'")) {
                Log.i("ContactTable", "UpdateQuery");
                row_id = (int) db.update("contact", cv, "userid='" + bean.getId() + "'and loginuser='" + name + "'", null);
            } else {
...
//                cv.put("toUserId", listToDetails1.getId());
//                    cv.put("signalid", signal_id);
                    if (listTaskConversationFile.getFileType().equals("text") || listTaskConversationFile.getFileType().equals("map") || listTaskConversationFile.getFileType().equals("date") || listTaskConversationFile.getFileType().equals("Reassign")) {
                        if (bean.getProjectId() != null) {
...
//                        cv.put("taskReceiver", listToDetails1.getUsername());
                        ...
                        ArrayList<String> grouplist_2 = VideoCallDataBase.getDB(context).selectGroupmembers("select * from contact", "username");
                        if (grouplist_2.size() > 0) {
                            if (!listToDetails1.getUsername().equalsIgnoreCase(Appreference.loginuserdetails.getUsername())) {
...
//    public ArrayList<String> select
    ...
    public int getUserId(int id) {
        Cursor cur = null;
        Integer from_userId = 0;
        try {
            if (!db.isOpen())
                openDatabase();
            String query = "select userid from username where userid='" + id + "'";
            cur = db.rawQuery(query, null);
            cur.moveToFirst();
            while (!cur.isAfterLast()) {
...
//            cv.put("callid", bean.getType());
            ...
            cv.put("fileName", taskDetailsBean.getServerFileName());
            ...
            cv.put("signature", taskDetailsBean.getCustomerSignature());
